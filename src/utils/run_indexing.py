...
async def run(persist_path="nq_corpus", collection_name="nq_corpus"):
    try:
        # load existing index
    ...
    ds = load_dataset("TIGER-Lab/LongRAG", "nq", split="subset_100", num_proc=8)

    for i, item in enumerate(tqdm(ds, desc="Indexing"), start=1):
        try:
            # converts dir to Document
            docs = Document(id_=item["query_id"], text=item["context"])
            ...
            index.insert_nodes(small_nodes)
            if i % 20 == 0:
